...
<h3 align="center">
  Demo :- <a href="https://to.amanstark.com/autoThemeChanger">Click Here</a>
</h3>
...
2. If you're going to use the Auto Theme Changer codes with some of my other codes, like [Avatar.js](https://to.amanstark.com/avatarjs-code), [Organic Safelink](https://to.amanstark.com/safelink), [ImgShield](https://to.amanstark.com/imgshield-codes), and so on, then merge all the configs of each code into a single config and add that to your site. **(No longer needed as the structure of the config for the Auto Theme Changer has been changed, and it is now `const ThemeManagerConfig` instead of `const config`.)**
